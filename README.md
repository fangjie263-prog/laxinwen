...
| 本地目录 | `YYYY-MM-DD/Ticker_Company/`，例如 `2026-09-12/09696.HK_天齐锂业/` |
...
研究报告与新闻归档**互相隔离**，不会把日期页 / 公司页混进新闻来源目录：

```
Laxinwen News                     ← NOTION_ROOT_PAGE_ID
├── 研究报告                       ← 固定中文一级目录（已存在则复用）
│   └── 2026-09-12                 ← 日期页（同一日期只创建一个）
│       └── 09696.HK｜天齐锂业      ← 公司页（同一公司只创建一个）
│           ├── Claude｜天齐锂业    ← 每个研究文件一个独立页面
│           └── Gemini｜天齐锂业｜B
├── RFI / ECO / HKEJ …             ← 新闻来源目录（结构不变）
```

- 一级目录固定为 `研究报告`（不使用 `Research` / `Research Archive` 等英文名）。
- 研究文件页面正文**极简**：`2026-09-12 · 000660.KS` + `📎 研究报告` + 文件附件。
- 标题格式 `AI｜Company`；同一天同一 AI 有多份文件时追加规范文件名里的序号字母
  （`Gemini｜SK海力士｜B`），稳定可预测、不依赖 SHA256。
- `SHA256` / 文件大小 / 分片 / 原始文件名 / 命中方式等技术元数据只保存在 SQLite，
  不再展示在 Notion 页面上。
- 已有旧页面不会被自动删除或迁移。
